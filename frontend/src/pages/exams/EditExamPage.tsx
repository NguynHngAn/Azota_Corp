...
export function EditExamPage() {
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    try {
      const desiredIsDraft = state.is_draft;
      const isDraftForEditing = true; // Keep exam editable while updating questions.
      const nextIsDraft = desiredIsDraft ? true : false;
      await updateExam(
        examId,
        {
          title: state.title.trim(),
          description: state.description.trim() || null,
          is_draft: isDraftForEditing,
    ...
    <div>
      ...
      {bankOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-card text-foreground rounded-2xl shadow-lg w-full max-w-3xl p-5 border border-border max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <Card className="mt-4 flex-1 min-h-0 border border-border shadow-sm hover:shadow-sm overflow-y-auto">
              {bankLoading ? (
